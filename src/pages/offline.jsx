...
const Watch = () => {
    ...
    const nextEpisodeAiringTime = animeInfo && animeInfo.nextAiringEpisode
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                console.log("episodes:: " ,animeData);
                if (isMounted && animeData) {
                    const transformedEpisodes = animeData
                        .filter((ep) => ep.episodes.includes('-episode-')) // Continue excluding entries without '-episode-'
                        .map((ep) => {
                        const episodePart = ep.episodes.split('-episode-')[1];
    ...
    return (<WatchContainer>
      {animeInfo &&
            ...
            animeInfo.trailer ? (<div style={{ textAlign: 'center' }}>
          <strong>
      ...
      <DataWrapper>
        <SourceAndData $videoPlayerWidth={videoPlayerWidth}>
          {animeInfo && animeInfo.status !== 'Not yet aired' && (<MediaSource sourceType={sourceType} setSourceType={setSourceType} language={language} setLanguage={setLanguage} downloadLink={downloadLink} episodeId={currentEpisode.number.toString()} airingTime={animeInfo && animeInfo.status === 'Ongoing'
                ? countdown
