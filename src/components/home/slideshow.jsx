...
import SkeletonLoader from '../skeletons/skeletons'; // Import SkeletonLoader component
...
const Slideshow = ({ data }) => {
  ...
  return (
    <div className="relative h-[65vh] mt-16 md:h-[35vh]" {...handlers}>
      {data.length === 0 ? ( // Render skeleton loader if data is empty
        <SkeletonLoader />
      ) : (
        data.map((item, index) => (
          <div
          ...
          >
            ...
            <div className={`absolute flex flex-col justify-end bottom-0 left-0 h-[100%] w-[80%] md:w-full p-16 bg-gradient-to-r from-black`}>
              <h2 className="text-[44px] md:text-[24px] font-bold opacity-100" onClick={() => handlePlayButtonClick(item.id)}>{truncateTitle(item.title, 45)}</h2>
